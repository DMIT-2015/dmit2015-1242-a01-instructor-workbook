...
public class StudentCrudView implements Serializable {
    ...
    public void onGenerateData() {
        try {
            var faker = new Faker();
            selectedStudent.setFirstName(faker.pokemon().name());
            selectedStudent.setLastName(faker.pokemon().type());
            // selectedStudent.setProperty3(faker.providerName().methodName());
